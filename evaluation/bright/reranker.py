...
from utils import get_device_with_most_free_memory
...
class Reranker:
    def __init__(self, task, device_count=None, device=None):
        ...
        self.sampling_params = SamplingParams(
            temperature=0.6, top_p=0.9, max_tokens=32, logprobs=5
        )
        self.model = LLM(
            model=model_name,
            dtype="bfloat16",
    ...
    def rerank(self, docs, query, topk, batch_size=50):
        ...
        for i in tqdm(range(0, len(docs), batch_size)):
            ...
            doc_prompts = [
                self.prompt.format(query, doc["text"]) for doc in list_docs
            ]
            ...
            for j in range(len(doc_prompt_outputs)):
                ...
                if pos_score != -1:
                    try:
                        ...
                        score = (
                            float(
                                doc_prompt_outputs[j][
                                    pos_score + 16 : pos_score + 18
                                ]
                            )
                            / 5
                        )
...
if __name__ == "__main__":
    ...
    if args.reasoning is not None:
        raw_examples = load_dataset(
            "xlangai/bright",
            f"{args.reasoning}_reason",
            cache_dir=args.cache_dir,
        )[args.task]
    else:
        raw_examples = load_dataset(
            "xlangai/bright", "examples", cache_dir=args.cache_dir
        )[args.task]
    ...
    if args.long_context:
        doc_pairs = load_dataset(
            "xlangai/bright", "long_documents", cache_dir=args.cache_dir
        )[args.task]
    else:
        doc_pairs = load_dataset(
    ...
    if not os.path.isfile(score_file_path):
        ...
        for qid, scores in tqdm(all_scores.items()):
            ...
            ctxs = [
                {"id": did, "text": documents[did]} for did, _ in sorted_scores
            ]
    ...
    results = calculate_retrieval_metrics(
        results=new_scores, qrels=ground_truth
    )
    ...
    for qid in new_scores:
        ...
        for did in new_scores[qid]:
            retriever_interpolated_scores[qid][did] = (
                0.5 * new_scores[qid][did]
            ) + (0.5 * all_scores[qid][did])
    results = calculate_retrieval_metrics(
        results=retriever_interpolated_scores, qrels=ground_truth
    )
    with open(
        os.path.join(outputs_path, f"reranker_retriever_results.json"), "w"
    ) as f:
        json.dump(results, f, indent=2)
    ...
    if args.bm25_score_file is not None:
        ...
        for qid in new_scores:
            ...
            for did in new_scores[qid]:
                bm25_interpolated_scores[qid][did] = (
                    100 * new_scores[qid][did]
                ) + bm25_scores[qid][did]
        results = calculate_retrieval_metrics(
            results=bm25_interpolated_scores, qrels=ground_truth
        )
        with open(
            os.path.join(outputs_path, f"reranker_bm25_results.json"), "w"
        ) as f:
            json.dump(results, f, indent=2)
