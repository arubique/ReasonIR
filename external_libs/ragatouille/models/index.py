...
class PLAIDModelIndex(ModelIndex):
    ...
    def search(
        ...
        **kwargs,
        ...
        if isinstance(query, str):
            ...
            longest_query_length = max(
                [int(len(x.split(" ")) * 1.35) for x in query]
            )
            self._upgrade_searcher_maxlen(
                longest_query_length, base_model_max_tokens
            )
    ...
    def _should_rebuild(current_len: int, new_doc_len: int) -> bool:
        ...
        """
        return (
            current_len + new_doc_len < 5000 or new_doc_len > current_len * 0.05
        )
    ...
    def add(
        ...
        **kwargs,
        ...
        if PLAIDModelIndex._should_rebuild(
            ...
            ):  # Update bsize if it's different
                self.config.index_bsize = bsize
    ...
    def delete(
        ...
        updater = IndexUpdater(
            config=config, searcher=searcher, checkpoint=checkpoint
        )
