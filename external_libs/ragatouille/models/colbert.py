...
class ColBERT(LateInteractionModel):
    def __init__(
        ...
        **kwargs,
        ...
        if load_from_index:
            ...
            self.index_root = (
                index_root if index_root is not None else ".ragatouille/"
            )
    ...
    def _get_collection_files_from_disk(self, index_path: str):
        ...
        try:
            self.pid_docid_map = srsly.read_json(
                str(index_path / "pid_docid_map.json")
            )
    ...
    def add_to_index(
        ...
        self.index_name = (
            index_name if index_name is not None else self.index_name
        )
        ...
        if self.loaded_from_index:
            ...
            if not self.collection:
                collection_path = (
                    Path(index_root) / self.index_name / "collection.json"
                )
        ...
        for idx, doc in enumerate(
            new_documents_with_ids, start=max_existing_pid + 1
        ):
    ...
    def _write_collection_files_to_disk(self):
        ...
        if self.docid_metadata_map is not None:
            srsly.write_json(
                self.index_path + "/docid_metadata_map.json",
                self.docid_metadata_map,
            )
    ...
    def train(self, data_dir, training_config: ColBERTConfig):
        training_config = ColBERTConfig.from_existing(
    ...
    def _index_free_retrieve(
        ...
        if k > len(documents):
            print(
                "k value cannot be larger than the number of documents! aborting..."
            )
            return None
    ...
    def encode(
        ...
        if hasattr(self, "in_memory_collection"):
            if self.in_memory_metadata is not None:
                ...
                self.in_memory_metadata = [None] * len(
