...
class TrainingDataProcessor:
    ...
    def _make_individual_triplets(self, query, positives, negatives):
        ...
        if len(positives) > 1:
            ...
            negs_per_positive = max(
                1, max_triplets_per_query // len(all_pos_texts)
            )
            ...
            extra_triplets_needed = (
                max_triplets_per_query - initial_triplets_count
            )
    ...
    def _get_new_negatives(
        self, query, passages, mine_hard_negatives, n_new_negatives
    ):
        ...
        if mine_hard_negatives:
            ...
            candidates = [
                ...
                if x not in passages["positives"]
                and x not in passages["negatives"]
            ]
    ...
    def _process_raw_pairs(
        self, raw_data, mine_hard_negatives, n_new_negatives
    ):
    ...
    def export_training_data(self, path: Union[str, Path]):
        ...
        srsly.write_jsonl(
            path / "triples.train.colbert.jsonl", self.training_triplets
        )
