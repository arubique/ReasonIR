...
class RAGTrainer:
    ...
    def prepare_training_data(
        ...
        if all_documents is not None:
            self.collection += [
                doc for doc in all_documents if isinstance(doc, str)
            ]
        ...
        if len(sample) == 2:
            ...
            raise ValueError(
                "Raw data must be a list of pairs or triplets of strings."
            )
    ...
    def train(
        ...
        if not self.training_triplets:
            total_triplets = sum(
                1
                for _ in open(
                    str(self.data_dir / "triples.train.colbert.jsonl"), "r"
                )
            )
        else:
        ...
        return self.model.train(
            data_dir=self.data_dir, training_config=training_config
        )
