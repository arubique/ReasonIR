...
class StridedTensor(StridedTensorCore):
    ...
    def pad_packed(cls, packed_tensor, lengths):
        ...
        packed_tensor, lengths = (
            packed_tensor.cuda().contiguous(),
            lengths.cuda(),
        )
    ...
    def lookup_packed_unordered(self, pids):
        ...
        order = torch.arange(
            pids.size(0), device="cuda" if self.use_gpu else "cpu"
        )
...
if __name__ == "__main__":
    ...
    index_path = (
        "/future/u/okhattab/root/unit/indexes/2021/08/residual.NQ-micro"
    )
    with open(
