    ...
    def set_embeddings_strided(self):
        if self.load_index_with_mmap:
            ...
            self.offsets = torch.cat(
                (torch.zeros(1, dtype=torch.int64), self.offsets)
            )
        else:
    ...
    def rank(self, config, Q, filter_fn=None, pids=None):
        with torch.inference_mode():
            if pids is None:
                ...
                pids = torch.tensor(
                    pids, dtype=pids_.dtype, device=pids_.device
                )

            if filter_fn is not None:
                filtered_pids = filter_fn(pids)
                assert isinstance(filtered_pids, torch.Tensor), type(
                    filtered_pids
                )
                assert (
        ...
        if self.use_gpu:
            ...
            for i in range(0, ceil(len(pids) / batch_size)):
                ...
                (
                    codes_packed,
                    codes_lengths,
                ) = self.embeddings_strided.lookup_codes(pids_)
