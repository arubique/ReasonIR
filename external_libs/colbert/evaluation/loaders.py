...
def load_topK(topK_path):
    ...
    print_message(
        "#> max(Ks) =", max(Ks), ", avg(Ks) =", round(sum(Ks) / len(Ks), 2)
    )
    print_message(
...
def load_topK_pids(topK_path, qrels):
    ...
    if len(topK_positives) == 0:
        ...
        for qid in set.difference(
            set(topK_pids.keys()), set(topK_positives.keys())
        ):
            topK_positives[qid] = []
        ...
        avg_positive = round(
            sum(len(topK_positives[qid]) for qid in topK_positives)
            / len(topK_pids),
            2,
        )
