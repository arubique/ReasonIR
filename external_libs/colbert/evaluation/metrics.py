...
class Metrics:
    ...
    def add(self, query_idx, query_key, ranking, gold_positives):
        ...
        for depth in self.mrr_sums:
            ...
            self.mrr_sums[depth] += (
                (1.0 / (first_positive + 1.0))
                if first_positive < depth
                else 0.0
            )
        ...
        for depth in self.recall_sums:
            ...
            self.recall_sums[depth] += num_positives_up_to_depth / len(
                gold_positives
            )
    ...
    def log(self, query_idx):
        ...
        Run.log_metric(
            "ranking/num_queries_added", self.num_queries_added, query_idx
        )
