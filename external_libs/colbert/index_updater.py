...
class IndexUpdater:
    ...
    def add(self, passages):
        ...
        print_message(
            f"#> Added {len(passages)} passages from pid {start_pid}."
        )
    ...
    def persist_to_disk(self):
        ...
        emb_last = (
            emb_start
            + torch.sum(self.searcher.ranker.doclens[pid_start:]).item()
        )
        ...
        if last_chunk_capacity > 0:
            ...
            emb_end = (
                emb_start
                + torch.sum(
                    self.searcher.ranker.doclens[pid_start:pid_end]
                ).item()
            )
    ...
    def _load_chunk_doclens(self, chunk_idx):
        ...
        with open(
            os.path.join(self.index_path, f"doclens.{chunk_idx}.json")
        ) as f:
    ...
    def _load_chunk_residuals(self, chunk_idx):
        residuals_path = os.path.join(
            self.index_path, f"{chunk_idx}.residuals.pt"
        )
        return torch.load(residuals_path, map_location="cpu")

    def _load_chunk_metadata(self, chunk_idx):
        with open(
            os.path.join(self.index_path, f"{chunk_idx}.metadata.json")
        ) as f:
    ...
    def _get_chunk_idx(self, pid):
        for i in range(self.metadata["num_chunks"]):
            ...
            if (
                ...
                and chunk_metadata["passage_offset"]
                + chunk_metadata["num_passages"]
                > pid
            ):
                return i
    ...
    def _add_pid_to_ivf(self, partitions, pid, old_ivf, old_ivf_lengths):
        ...
        for i in range(len(old_ivf_lengths)):
            ...
            new_ivf.extend(
                old_ivf[ivf_runner : ivf_runner + old_ivf_lengths[i]]
            )
    ...
    def _write_to_last_chunk(self, pid_start, pid_end, emb_start, emb_end):
        ...
        print_message(
            f"#> Writing {pid_end - pid_start} passages to the last chunk..."
        )
        ...
        curr_doclens.extend(
            self.searcher.ranker.doclens.tolist()[pid_start:pid_end]
        )
        doclens_path = os.path.join(
            self.index_path, f"doclens.{num_chunks - 1}.json"
        )
    ...
    def _write_to_new_chunk(
        self, chunk_idx, pid_start, pid_end, emb_start, emb_end
    ):
        # Helper function for IndexUpdater.persist_to_disk()
        ...
        doclens_path = os.path.join(
            self.index_path, f"doclens.{chunk_idx}.json"
        )
        with open(doclens_path, "w+") as output_doclens:
            ujson.dump(curr_doclens, output_doclens)
    ...
    def _remove_passage_from_disk(self, pid):
        ...
        for idx in range(chunk_idx + 1, self.metadata["num_chunks"]):
            ...
            metadata_path = os.path.join(
                self.index_path, f"{idx}.metadata.json"
            )
