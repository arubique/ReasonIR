...
class ResidualCodec:
    ...
    def __init__(
        ...
        self.reversed_bit_map = torch.tensor(self.reversed_bit_map).to(
            torch.uint8
        )
        ...
        if self.use_gpu:
            ...
            if self.decompression_lookup_table is not None:
                self.decompression_lookup_table = (
                    self.decompression_lookup_table.cuda()
                )
    ...
    def decompress(self, compressed_embs: Embeddings):
        ...
        for codes_, residuals_ in zip(
            codes.split(1 << 15), residuals.split(1 << 15)
        ):
            if self.use_gpu:
