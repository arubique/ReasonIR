...
class ResidualEmbeddings:
    ...
    def load_chunks(
        ...
        if load_index_with_mmap:
            ...
            print_message(
                "#> Loading codes and residuals with memory mapping..."
            )
            ...
            residuals_size, codes_size, packed_dim = get_residuals_size(
                index_path, 0
            )
            ...
            residuals = torch.empty(
                num_embeddings, dim // 8 * nbits, dtype=torch.uint8
            )
    ...
    def save(self, path_prefix):
        ...
        residuals_path = (
            f"{path_prefix}.residuals.pt"  # f'{path_prefix}.residuals.bn'
        )
