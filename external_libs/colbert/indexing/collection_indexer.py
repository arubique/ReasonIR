...
def encode(config, collection, shared_lists, shared_queues, verbose: int = 3):
    encoder = CollectionIndexer(
        config=config, collection=collection, verbose=verbose
    )
...
class CollectionIndexer:
    ...
    def run(self, shared_lists):
        with torch.inference_mode():
            ...
            if not self.config.resume or not self.saver.try_load_codec():
                self.train(
                    shared_lists
                )  # Trains centroids from selected passages
            distributed.barrier(self.rank)
            print_memory_stats(f"RANK:{self.rank}")
        ...
        if self.config.resume:
            if self._try_load_plan():
                if self.verbose > 1:
                    ...
                    Run().print_main(
                        f"#> avg_doclen_est = {self.avg_doclen_est}"
                    )
                return
    ...
    def _sample_pids(self):
        ...
        typical_doclen = (
            120  # let's keep sampling independent of the actual doc_maxlen
        )
    ...
    def _concatenate_and_split_sample(self):
        ...
        sample = torch.empty(
            self.num_sample_embs, self.config.dim, dtype=torch.float16
        )
    ...
    def _compute_avg_residual(self, centroids, heldout):
        ...
        quantiles = torch.arange(
            0, num_options, device=heldout_avg_residual.device
        ) * (1 / num_options)
        bucket_cutoffs_quantiles, bucket_weights_quantiles = quantiles[
            1:
        ], quantiles + (0.5 / num_options)

        bucket_cutoffs = heldout_avg_residual.float().quantile(
            bucket_cutoffs_quantiles
        )
        bucket_weights = heldout_avg_residual.float().quantile(
            bucket_weights_quantiles
        )
    ...
    def _update_metadata(self):
        ...
        if self.verbose > 1:
            Run().print(
                "#> Saving the indexing metadata to", self.metadata_path, ".."
            )
