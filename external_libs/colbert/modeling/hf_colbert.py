...
def class_factory(name_or_path):
    loadedConfig = AutoConfig.from_pretrained(
    ...
    if getattr(loadedConfig, "auto_map", None) is None:
        ...
        model_class_object = get_class_from_dynamic_module(
            model_class, name_or_path
        )
    ...
    class HF_ColBERT(pretrained_class_object):
        ...
        def from_pretrained(cls, name_or_path, colbert_config):
            ...
            obj = super().from_pretrained(
                name_or_path, colbert_config=colbert_config
            )
