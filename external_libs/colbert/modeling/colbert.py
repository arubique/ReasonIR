...
class ColBERT(BaseColBERT):
    ...
    def __init__(self, name="bert-base-uncased", colbert_config=None):
        ...
        if self.colbert_config.mask_punctuation:
            self.skiplist = {
                ...
                for w in [
                    symbol,
                    self.raw_tokenizer.encode(symbol, add_special_tokens=False)[
                        0
                    ],
                ]
            }
    ...
    def forward(self, Q, D):
        ...
        Q_duplicated = Q.repeat_interleave(
            self.colbert_config.nway, dim=0
        ).contiguous()
...
def colbert_score_reduce(scores_padded, D_mask, config: ColBERTConfig):
    D_padding = ~D_mask.view(
        scores_padded.size(0), scores_padded.size(1)
    ).bool()
