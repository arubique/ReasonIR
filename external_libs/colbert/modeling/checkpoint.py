...
def pool_embeddings_hierarchical(
    ...
    for token_length in tqdm(token_lengths, desc="Pooling tokens"):
        ...
        max_clusters = (
            token_length // pool_factor
            if token_length // pool_factor > 0
            else 1
        )
        ...
        for cluster_id in range(1, max_clusters + 1):
            ...
            if cluster_indices.numel() > 0:
                pooled_embedding = passage_embeddings[cluster_indices].mean(
                    dim=0
                )
        ...
        pooled_token_lengths.append(
            len(pooled_embeddings) - sum(pooled_token_lengths)
        )
...
class Checkpoint(ColBERT):
    ...
    def __init__(self, name, colbert_config=None, verbose: int = 3):
        ...
        self.query_tokenizer = QueryTokenizer(
            self.colbert_config, verbose=self.verbose
        )
    ...
    def docFromText(
        ...
        if bsize:
            ...
            if return_tokens:
                ...
                returned_text = [
                    returned_text[idx] for idx in reverse_indices.tolist()
                ]
            ...
            if keep_dims is True:
                ...
                if pool_factor > 1:
                    print(
                        f"Clustering tokens with a pool factor of {pool_factor}"
                    )
            ...
            return (
                [D[idx] for idx in reverse_indices.tolist()],
                *returned_text,
            )
