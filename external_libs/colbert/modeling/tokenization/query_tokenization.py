...
class QueryTokenizer:
    def __init__(self, config: ColBERTConfig, verbose: int = 3):
        ...
        ) = config.query_token, self.tok.convert_tokens_to_ids(
    ...
    def encode(self, batch_text, add_special_tokens=False):
        ...
        ids = self.tok(batch_text, add_special_tokens=False).to(DEVICE)[
            "input_ids"
        ]
    ...
    def tensorize(
        self, batch_text, bsize=None, context=None, full_length_search=False
    ):
        assert type(batch_text) in [list, tuple], type(batch_text)
        ...
        if full_length_search:
            ...
            un_truncated_ids = self.tok(
                batch_text, add_special_tokens=False
            ).to(DEVICE)["input_ids"]
        ...
        if context is not None:
            assert len(context) == len(batch_text), (
                len(context),
                len(batch_text),
            )
        ...
        if self.used is False:
            ...
            if self.verbose > 1:
                ...
                print(
                    f"#> Input: {batch_text[0]}, \t\t {firstbg}, \t\t {bsize}"
                )
